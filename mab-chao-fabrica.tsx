...
const INSPECAO_VISUAL_ITEMS = [
  'Ausência de trincas visíveis',
  'Ausência de deformações severas',
  'Ausência de corrosão excessiva',
  'Ausência de soldas inadequadas',
  'Condição geral aceitável para desmontagem',
];
...
const nowTimeISO = () => {
  ...
  return `${todayISO()} ${String(d.getHours()).padStart(2, '0')}:${String(
    d.getMinutes()
  ).padStart(2, '0')}`;
};
...
/* ---------------------------------------------------------------------- */
/* SUPABASE — SALVAMENTO CENTRALIZADO                                      */
/* ---------------------------------------------------------------------- */
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl mb-4 overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3 bg-slate-50 border-b border-slate-200">
        {numero && (
          <span className="font-display text-xl text-amber-600 w-8">
            {numero}
          </span>
        )}
        ...
        <div>
          <h3 className="font-display text-sm uppercase tracking-wide text-slate-800">
            {title}
          </h3>
...
const inputCls =
  "w-full px-3 py-2.5 border border-slate-300 rounded-lg text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500";
    ...
    <div>
      ...
      <div className="flex gap-2 mt-2">
        <button
          type="button"
          onClick={clear}
          className="flex items-center gap-1 text-xs text-slate-500 px-3 py-1.5 border border-slate-300 rounded-lg"
        >
          <RotateCcw size={13} />
          Limpar
        </button>
    ...
    <div className="w-20 md:w-24 bg-slate-900 flex flex-col items-center py-4 shrink-0 no-print">
      ...
      {items.map((it) => (
        <button
          ...
          className={`flex flex-col items-center gap-1 w-full py-3 mb-1 transition-colors ${
            view === it.id
              ? 'text-amber-400'
              : 'text-slate-500'
          }`}
        >
          ...
          <span className="font-body text-[9px] leading-tight text-center px-1">
            {it.label}
          </span>
        </button>
      ))}
...
function HomeView({
  ...
  const opsAbertas = ops.filter(
    (o) => !o.liberacaoFinal?.status
  );

  const opsConcluidas = ops.filter(
    (o) => o.liberacaoFinal?.status
  );
  ...
  return (
    <div className="font-body">
      ...
      <p className="text-sm text-slate-400 mb-6">
        MAB Embreagem · Registros ISO de remanufatura
      </p>

      <div className="grid grid-cols-2 gap-3 mb-6">
        <button
          onClick={() => setView('recebimento')}
          className="text-left bg-white border border-slate-200 rounded-xl p-4 hover:border-amber-400 transition-colors"
        >
          <ClipboardList
          ...
          <div className="font-display text-sm text-slate-800">
            Novo Recebimento
          </div>

          <div className="text-xs text-slate-400">
            Checklist de triagem de carcaças
          </div>
        </button>
      ...
      <h2 className="font-display text-sm uppercase text-slate-500 mb-2">
        Atividade recente
      </h2>
...
function OperadoresView({
  ...
  return (
    <div className="font-body max-w-xl">
      ...
      <p className="text-sm text-slate-400 mb-6">
        Cadastre uma vez — a assinatura fica salva e é usada em todos os registros.
      </p>
      ...
      <div className="space-y-2 mb-4">
        {operadores.map((o) => (
          ...
          >
            <img
              src={o.assinatura}
              className="h-10 w-20 object-contain bg-slate-50 rounded border border-slate-100"
            />
      ...
      {!adding ? (
        <button
          ...
          className="flex items-center gap-2 text-sm font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5"
        >
          <Plus size={16} />
          Cadastrar operador
        </button>
        ...
        >
          ...
          <Field label="Função">
            <input
              ...
              placeholder="Ex: Operador de desmontagem"
            />
          </Field>
          ...
          <div className="flex gap-2">
            <button
              onClick={salvar}
              disabled={!nome.trim() || !assinatura}
              className="flex-1 bg-slate-800 disabled:opacity-30 text-white text-sm font-semibold py-2.5 rounded-lg"
            >
              Salvar operador
            </button>
            ...
            <button
              ...
              className="px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-500"
            >
              Cancelar
            </button>
          </div>
...
function novoRecebimentoState() {
  return {
    ...
    conferencia:
      CONFERENCIA_RECEBIMENTO_ITEMS.map(
        (desc) => ({
          desc,
          status: 'pos',
          obs: ''
    ...
    <div className="font-body max-w-2xl">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h1 className="font-display text-2xl text-slate-800">
          Checklist de Recebimento e Triagem
        </h1>
      </div>
      ...
      >
        ...
        <button
          ...
          className="text-xs font-semibold text-amber-700 flex items-center gap-1 mt-1"
        >
          <Plus size={13} />
          Adicionar modelo
        </button>
      ...
      <SectionCard
        title="1. Conferência de recebimento"
        icon={CheckCircle2}
      >
        {f.conferencia.map(
          (item, i) => (
      ...
      <SectionCard
        title="2. Inspeção visual inicial"
        icon={Search}
      >
        {f.inspecao.map(
          (item, i) => (
      ...
      <SectionCard
        title="5. Classificação da carcaça"
        icon={FileText}
      >
        <div className="flex gap-2">
          {[
            {
              v: 'A',
              l: 'Classe A · Reutilização imediata'
            },
            {
              v: 'B',
              l: 'Classe B · Necessita recuperação'
            },
            {
              v: 'C',
              l: 'Classe C · Sucata'
            ...
            <button
              ...
              className={`flex-1 text-xs font-semibold py-3 rounded-lg border ${
                f.classe === c.v
      ...
      >
        <OperadorPicker
          ...
          label="Recebido e conferido por"
        />

        <Field label="Data do recebedor">
          <input
            type="date"
            className={inputCls}
            value={f.dataRecebedor}
            onChange={(e) =>
              upd({
                dataRecebedor:
                  e.target.value
      ...
      <div className="flex gap-2 sticky bottom-0 bg-slate-100 py-3">
        <button
          ...
          className="flex-1 bg-amber-600 disabled:opacity-30 text-white font-display text-sm py-3 rounded-lg"
        >
          Salvar registro
        </button>
        ...
        <button
          onClick={onCancelar}
          className="px-5 border border-slate-300 rounded-lg text-sm text-slate-500"
        >
          Cancelar
        </button>
      </div>
...
function EtapaBlock({
  ...
  return (
    ...
    >
        ...
        <Field label="Início (data/hora)">
          <div className="flex gap-1">
            ...
            <button
              ...
              className="px-2 border border-slate-300 rounded-lg text-slate-400 shrink-0"
            >
              <Clock size={16} />
            </button>
          </div>
          ...
          (m, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <input
                  className="flex-1 text-xs font-medium text-slate-500 uppercase tracking-wide bg-transparent border-b border-slate-200 focus:outline-none focus:border-amber-500 py-1"
                  value={m.refLabel}
                  onChange={(e) =>
                    updMed(
                      i,
      ...
      <OperadorPicker
        operadores={operadores}
        value={etapa.operadorId}
        onChange={(v) =>
          onChange({
            operadorId: v
          })
        }
        label="Visto operador"
      />
    </SectionCard>
  );
}
  ...
  return (
    <div className="font-body max-w-2xl">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h1 className="font-display text-2xl text-slate-800">
          Ordem de Produção — Remanufatura
        </h1>
      </div>
          ...
          <div className="flex gap-2">
            {[
              ...
              <button
                ...
                className={`flex-1 text-xs font-semibold py-2.5 rounded-lg border ${
                  f.novaReman ===
                  v
                    ? 'bg-slate-800 text-white border-slate-800'
                    : 'bg-white text-slate-500 border-slate-300'
      ...
      <SectionCard
        title="Observações / Não Conformidades"
        icon={AlertCircle}
      >
        <textarea
          className={inputCls}
      ...
      >
        {f.requisicoes.map(
          (r, i) => (
            <div
              key={i}
              className="border border-slate-100 rounded-lg p-2 mb-2"
            >
      ...
      <SectionCard
        title="Liberação final (Controle de Qualidade)"
        icon={CheckCircle2}
      >
...
function HistoricoView({
  ...
  return (
    <div className="font-body max-w-2xl">
      <h1 className="font-display text-2xl text-slate-800 mb-4">
        Histórico
      </h1>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Buscar por OP, lote, modelo, cliente..."
          value={busca}
          onChange={(e) =>
            setBusca(
              e.target.value
    ...
    <div className="font-body max-w-2xl print-area">
      <div className="flex items-center justify-between mb-4 no-print">
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs font-semibold text-slate-500 border border-slate-300 rounded-lg px-3 py-2"
        >
          <Printer size={14} />
          Imprimir / PDF
        </button>
...
function EmptyOperadores({
  setView
}) {
  return (
    <div className="max-w-md font-body">
      <h2 className="font-display text-xl text-slate-800 mb-2">
        Cadastre um operador primeiro
      </h2>

      <p className="text-sm text-slate-500 mb-4">
        Para colher a assinatura automaticamente nos formulários, é preciso cadastrar ao menos um operador.
      </p>
      ...
      <button
        ...
        className="bg-amber-600 text-white text-sm font-semibold px-4 py-2.5 rounded-lg"
      >
        Ir para Operadores
      </button>
    </div>
